...
def main() -> None:
    ...
    if TARGET_QF not in files_by_qf:
        raise ValueError(f"QF={TARGET_QF} not found in {ac_root}")
    ...
    plt.ylabel("PSNR (dB)")
    plt.title(f"AC encryption: aligned mean PSNR vs BPP (QF={TARGET_QF})")
    plt.grid(True)
    plt.legend()
    output_name = f"ac_psnr_vs_bpp_qf{TARGET_QF}_aligned.png"
