...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot aligned DC PSNR-BPP curve.")
    ...
    if args.target_qf not in files_by_qf:
        raise ValueError(f"QF={args.target_qf} not found in {dc_root}")
    ...
    plt.ylabel("PSNR (dB)")
    plt.title(f"DC encryption: aligned mean PSNR vs BPP (QF={args.target_qf})")
    plt.grid(True)
    plt.legend()
    output_path = args.output if args.output is not None else Path(f"dc_psnr_vs_bpp_qf{args.target_qf}_aligned.png")
